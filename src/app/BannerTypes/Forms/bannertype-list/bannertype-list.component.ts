...
import { BannertypeCuComponent } from '../bannertype-cu/bannertype-cu.component';

@Component({
  selector: 'app-bannertype-list',
  standalone: true,
  ...
  templateUrl: './bannertype-list.component.html',
  styleUrls: ['./bannertype-list.component.css']
})
export class BannertypeListComponent implements OnInit {

  vmsjur: bannertypesmdl[] = [];
  ...
  displayedColumns: string[] = ['nourut', 'cmbtId', 'cmbtType', 'cmbtFilename', 'cmbtFilepath', 'action'];
  vct: number = 0;
  events: string[] = [];
  month: string = "a";

  constructor(private _mbtserv: ServBannertypesService, private _router: Router, private dialog: MatDialog,
  ) {
  ...
  getBannersListAll() {
    ...
    this.vct = 0;
    //alert('productlist');
    this._mbtserv.getBanTypeListAll().subscribe((res: bannertypesmdl[]) => {
      // alert('bantpe: '+res.length)
      this.vmsjur = res;
  ...
  viewAddBanType(ptranstype: string, vprodcode: any) {
    const dialogRef = this.dialog.open(BannertypeCuComponent, { height: '60%', width: '65%' },);
